...
#define PageTable "./pt.bin"
#define PhysicalMem "./pm.bin"
#define Disk "./disk.bin"
#define inputfile "./input.bin"
...
__device__ __managed__ u32 pagetable[INVERT_PAGE_TABLE_SIZE];
__device__ __managed__ uchar phymem[PHYSICAL_MEM_SIZE];
__device__ __managed__ uchar secondary[STORAGE_SIZE];
...
__global__ void mykernel(int input_size) {
  ...
  if(threadIdx.x==0)user_program(&vm,input,results,input_size); 
  init_invert_page_table(&vm);
  __syncthreads();
  if(threadIdx.x==1)user_program(&vm,input,results,input_size); 
  init_invert_page_table(&vm);
  __syncthreads();
  if(threadIdx.x==2)user_program(&vm,input,results,input_size); 
  init_invert_page_table(&vm);
  __syncthreads();
  if(threadIdx.x==3)user_program(&vm,input,results,input_size); 
  init_invert_page_table(&vm);
  __syncthreads();
...
int main() {
  ...
  write_binaryFile(PageTable, pagetable, INVERT_PAGE_TABLE_SIZE);
  write_binaryFile(PhysicalMem, phymem, PHYSICAL_MEM_SIZE);
  write_binaryFile(Disk, secondary,STORAGE_SIZE);
  write_binaryFile(inputfile, input, input_size);
